...
/* Suricata's build sets __SCFILENAME__ per TU. Our Cargo build doesn't,
 * so default would be "undefined" in SCLog* output. */
#ifndef __SCFILENAME__
#define __SCFILENAME__ "ndpi-plugin"
#endif

#ifndef NDPI_PLUGIN_VERSION
#define NDPI_PLUGIN_VERSION "dev"
#endif

#include "suricata-compat.h"
...
static void OnFlowUpdate(ThreadVars *tv, Flow *f, Packet *p, void *_data)
{
    ...
    if (threadctx == NULL || flowctx == NULL || threadctx->ndpi == NULL || flowctx->ndpi_flow == NULL) {
        return;
    }
    ...
    if (!flowctx->detection_completed && ip_ptr != NULL && ip_len > 0) {
        ...
        if (ndpi_is_protocol_detected(flowctx->detected_l7_protocol) != 0) {
            if (!ndpi_is_proto_unknown(flowctx->detected_l7_protocol.proto)) {
                bool extra_done = flowctx->ndpi_flow->num_extra_packets_checked >=
                                  flowctx->ndpi_flow->max_extra_packets_to_check;
                if (flowctx->detected_l7_protocol.state == NDPI_STATE_CLASSIFIED || extra_done) {
...
static void OnThreadInit(ThreadVars *tv, void *_data)
{
    ...
    if (ndpi_finalize_initialization(context->ndpi) != 0) {
        FatalError("Failed to finalize nDPI detection module initialization");
    }
        ...
        DetectEngineThreadCtx *det_ctx, Packet *p, const Signature *s, const SigMatchCtx *ctx)
{
    ...
    struct NdpiFlowContext *flowctx = FlowGetStorageById(f, flow_storage_id);
    if (flowctx == NULL || !flowctx->detection_completed) {
        ...
        DetectEngineThreadCtx *det_ctx, Packet *p, const Signature *s, const SigMatchCtx *ctx)
{
    ...
    struct NdpiFlowContext *flowctx = FlowGetStorageById(f, flow_storage_id);
    if (flowctx == NULL || !flowctx->detection_completed || flowctx->ndpi_flow == NULL) {
...
static void NdpInitRiskKeyword(void)
{
    ...
    if (ndpi_protocol_keyword_id < 0) {
        FatalError("Failed to register ndpi-protocol keyword");
    }
    ...
    if (ndpi_risk_keyword_id < 0) {
        FatalError("Failed to register ndpi-risk keyword");
    }
...
static void NdpiInit(void)
{
    ...
    const char *revision = ndpi_revision();
    if (revision != NULL) {
        SCLogNotice("nDPI plugin loaded (nDPI revision: %s)", revision);
    } else {
        SCLogNotice("nDPI plugin loaded (nDPI revision unavailable)");
    }
}

static const SCPlugin PluginRegistration = {
    .version = SC_API_VERSION,
    .suricata_version = SC_PACKAGE_VERSION,
    .plugin_version = NDPI_PLUGIN_VERSION,
    .name = "ndpi",
    .author = "Luca Deri",
...
/* Rust exports SCPluginRegister and calls into this function. */
const SCPlugin *NdpiPluginRegister(void)
{
